...
fn main() -> Result<(), Box<dyn Error>> {
	...
	let session_options = slang::CompilerOptions::default().matrix_layout_column(true);
	...
	for module_name in ["back.slang", "main.slang"] {
		...
		let entry_point_v = module
			.find_entry_point_by_name("vertMain")
			.ok_or("No entrypoint found")?;
		let entry_point_f = module
			.find_entry_point_by_name("fragMain")
			.ok_or("No entrypoint found")?;
		...
		println!(
			"cargo::warning={:?}",
			target_profile_dir.join("shaders").join(module_name)
		);
