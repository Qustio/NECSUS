...
use super::{buffer::Buffer, vulkan_context::{Device, Allocator}};
		...
		let image = unsafe {
			allocator.create_image(
				&vk::ImageCreateInfo::default()
					...
					.extent(vk::Extent3D::default().width(extent.width).height(extent.height).depth(1))
...
impl Drop for FrameCapture {
	fn drop(&mut self) {
		unsafe {
			self.allocator.destroy_image(self.image.0, &mut self.image.1);
		}
	}
}
