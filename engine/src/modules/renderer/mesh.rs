...
impl<V> Mesh<V> {
	fn new(
		...
		let vertex_buffer = Buffer::<V>::from_slice(allocator.clone(), &vertexes, vk::BufferUsageFlags::VERTEX_BUFFER)?;
		let index_buffer = Buffer::<u32>::from_slice(allocator.clone(), &indicies, vk::BufferUsageFlags::INDEX_BUFFER)?;
...
impl VertexDescription for Vertex {
	...
	fn attributes() -> Vec<vk::VertexInputAttributeDescription> {
		vec![
			vk::VertexInputAttributeDescription::default().location(0).binding(0).format(vk::Format::R32G32B32A32_SFLOAT).offset(offset_of!(Vertex, pos) as u32),
			vk::VertexInputAttributeDescription::default().location(1).binding(0).format(vk::Format::R32G32B32_SFLOAT).offset(offset_of!(Vertex, normal) as u32),
			vk::VertexInputAttributeDescription::default().location(2).binding(0).format(vk::Format::R32G32B32A32_SFLOAT).offset(offset_of!(Vertex, color) as u32),
		]
	}
}

pub trait FromGLTF: Sized + Sync + Send {
    fn from_primitive(primitive: &gltf::Primitive, buffers: &[gltf::buffer::Data]) -> (Vec<Self>, Vec<u32>);
}
